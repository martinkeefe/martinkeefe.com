...
import xgui from '../lib/xgui'
import {$i,$one} from './lib'

//import cone from './parasurf/cone'
//import sphere from './parasurf/sphere'
//import torus from './parasurf/torus'
//import elliptic_torus from './parasurf/elliptic-torus'
//import limpet_torus from './parasurf/limpet-torus'
//import supertoroid from './parasurf/supertoroid'
//import mobius from './parasurf/mobius'
//import boys_1 from './parasurf/boys-1'
//
//
//var shapes = {
//	'cone': cone,
//	'sphere': sphere,
//	'torus': torus,
//	'elliptic-torus': elliptic_torus,
//	'limpet-torus': limpet_torus,
...
function parasurf(elem_id, shape_name){
	...
	var shape_def = require('./parasurf/'+shape_name).default;
	var shape = shape_def();
	var extras = [];
	...
	function XUI(elem_id) {
		$i('ps-shape').setAttribute('style', 'text-align: center');

		var elem = $i(elem_id);
		var canvas = $one('<canvas width=' + elem.scrollWidth + ' height=' + elem.scrollWidth*0.75 + '></canvas>');

		elem.appendChild(canvas);
		setup(canvas, canvas.scrollWidth, canvas.scrollHeight);
		...
		var gui_el = $one('<div style="width: 280px; margin: 0 auto;"></div>');
		gui_el.appendChild(gui.getDomElement());
		elem.appendChild(gui_el);
		...
		wire_chk.value.bind(v => (show_wires = v) ? group.add(wire_lines) : group.remove(wire_lines));
		y += dy;

		var face_chk = new gui.CheckBox( {x: x, y: y, text: "Surface", selected: true, width: 14, height:14} );
		face_chk.value.bind(v => (show_faces = v) ? group.add(body_mesh) : group.remove(body_mesh));
		y += dy;

		var axes_chk = new gui.CheckBox( {x: x, y: y, text: "Axes", selected: true, width: 14, height:14} );
		axes_chk.value.bind(v => (show_axes = v) ? group.add(axes) : group.remove(axes));
		y += dy;

		var xy_chk = new gui.CheckBox( {x: x, y: y, text: "XY Plane", selected: true, width: 14, height:14} );
		xy_chk.value.bind(v => (show_xyplane = v) ? group.add(xyplane) : group.remove(xyplane));
		y += dy;
		...
		extras.forEach(extra => {
			...
			ex_chks.push(chk);
			chk.value.bind(v => (extra.show = v) ? group.add(extra.group) : group.remove(extra.group));
			y += dy;
		});
