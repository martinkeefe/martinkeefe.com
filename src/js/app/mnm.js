...
function MondayPage(props : MondayProps) {
    const mnm  = (MNMS.find(val => val.num == props.match.params.mnm) : MNM)
    ...
    return (
        <NormalPage title={'Monday Night Martin - ' + mnm.title} date={mnm.date} ident="mnm">
            ...
            <p>{recs[0].trk_name || ''}</p>
            <PlayList key={mnm.num} mnm={mnm} recs={recs.slice(1)} />
        </NormalPage>
    )
}
